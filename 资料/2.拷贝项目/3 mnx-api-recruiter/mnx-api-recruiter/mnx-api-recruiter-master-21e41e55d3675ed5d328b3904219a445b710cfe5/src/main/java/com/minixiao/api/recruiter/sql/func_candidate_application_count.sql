...
--执行SQLZ语句
EXECUTE 'SELECT count(DISTINCT a.id) from tb_candidate_application a
 ...
 where '||f_recId||f_stage_id||f_name||f_positions||f_degree||f_school||f_major||f_status||f_labels||f_gender||f_department||f_englishLevel||f_rank||f_reward||f_attachment||f_mobile||f_city||f_birth_from||f_birth_to||f_graduate_from||f_graduate_to||f_delivery_from||f_delivery_to||';'
INTO total;
